...
RSpec.describe Championship, type: :model do
  subject(:championship) { FactoryBot.create(:championship) }
  ...
  it 'is invalid when max players is less than the minimum' do
    championship.min_players_per_team = 6
    championship.max_players_per_team = 5

    expect(championship).not_to be_valid
    expect(championship.errors[:max_players_per_team]).to include(/greater than or equal to/i)
  end
end
