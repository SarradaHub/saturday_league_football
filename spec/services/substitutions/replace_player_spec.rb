  ...
  describe '#call' do
    context 'when player is in round' do
      ...
      it 'adds replacement player to round if not already there' do
        ...
        expect {
          call_result
        }.not_to change { round.players.count } # -1 removed, +1 added = 0 net change
      end
      ...
      it 'triggers auto-balance via PlayerRound callbacks' do
        ...
        call_result
        expect(Rounds::RoundTeamGenerator).to have_received(:call).at_least(:once)
      end
    end

    context 'when player is not in round' do
      it 'raises PlayerNotInRoundError' do
        player_not_in_round = FactoryBot.create(:player)
        expect {
    ...
    context 'when no available player exists' do
      before do
        ...
        allow(described_class).to receive(:new).and_wrap_original do |method, *args|
          method.call(*args).tap { |instance| allow(instance).to receive(:active_match_team_ids).and_return([team1.id, team2.id]) }
        end
      end
    ...
    context 'when match_id is provided' do
      ...
      it 'excludes teams from active match when finding replacement' do
        ...
        result = described_class.call(round: round, player_id: player_to_remove.id, match_id: match.id)
